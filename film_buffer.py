...
#    Updated: 2018/11/11 15:21:19 by agissing         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
cookie = '-H "Cookie: user.id=' + user_id + '; _intra_42_session_production=' + session

page_url = 'https://elearning.intra.42.fr/videos?page='
url_list = ''

print ("\nPlease wait until all videos are loaded !\n")

for i in range(0, 37):
    cmd = 'curl --silent "' + page_url + str(i) + '" ' + user_agent + cookie + '" |  grep -o "/notions/[-a-zA-Z0-9@:%_\+\.~#?&\\/=]*" | sed -e "s/$/\/seen/"'
...
for url in url_list.split('\n'):
    cmd = 'curl --silent "https://elearning.intra.42.fr' + url + '" ' + user_agent + cookie + '"'
